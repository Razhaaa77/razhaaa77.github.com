...
class Galeri extends CI_Controller {
   ...
   public function simpan(){
    ...
    $config['upload_path']          = 'assets/flat_able/upload/galeri/';
    $config['max_size'] = 10000 * 1024; //3 * 1024 * 1024; //3Mb; 0=unlimited
    $config['file_name']            = $namafoto;
    $config['allowed_types']        = '*';
    $this->load->library('upload', $config);
    if($_FILES['foto_galeri']['size'] >= 10000 * 1024){
    ...
    }  elseif(! $this->upload->do_upload('foto_galeri')){
    ...
    public function hapus($id){
      $filename=FCPATH.'/assets/flat_able/upload/galeri/'.$id;
        if(file_exists($filename)){
          unlink("./assets/flat_able/upload/galeri/".$id);
        }
      $where = array(
        'foto_galeri'     => $id
      );
